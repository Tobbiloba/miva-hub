...
async function cleanup() {
  ...
  for (const r of realGst.rows) realMap.set(r.course_code as string, r.id as string);
  ...
  for (const r of dupeGst.rows) dupeMap.set(r.course_code as string, r.id as string);
  ...
  // Get program ID
  const progResult = await db.execute(sql`SELECT id FROM program WHERE code = 'CS' LIMIT 1`);
  ...
  try {
    ...
    if (dupeMap.has("GST201")) {
      ...
      console.log("\n1. Instructor reassigned: GST201 → GST112 (" + instrUpdate.rowCount + " rows)");
      ...
      console.log("2. Schedules reassigned: GST201 → GST112 (" + schedUpdate.rowCount + " rows)");
    }

    // 3. Delete curriculum mappings for ALL duplicate GSTs
    const currDel = await db.execute(
      sql`DELETE FROM program_curriculum WHERE course_id IN (${sql.join([...dupeMap.values()].map(id => sql`${id}::uuid`), sql`, `)})`
    );
    console.log("3. Curriculum mappings deleted: " + currDel.rowCount + " rows");

    // 4. Insert new curriculum mappings for real GST codes (skip if already exists)
    const mappings = [
      { courseId: realMap.get("GST111")!, level: 100, semester: "first", order: 3 },
      { courseId: realMap.get("GST122")!, level: 100, semester: "second", order: 3 },
      { courseId: realMap.get("GST112")!, level: 200, semester: "first", order: 5 },
      { courseId: realMap.get("GST212")!, level: 200, semester: "second", order: 5 },
    ];
    ...
    console.log("5. Duplicate courses deleted: " + courseDel.rowCount + " rows");
  ...
  for (const r of verify.rows) {
    const row = r as any;
    console.log("  Level " + row.level + " | " + row.semester + " | " + row.course_code + " | " + row.title);
  }
  ...
  for (const r of schedVerify.rows) {
    const row = r as any;
    console.log("  " + row.course_code + " | " + row.day_of_week + " " + row.start_time + "-" + row.end_time);
  }

  const remainCheck = await db.execute(
    sql`SELECT course_code FROM course WHERE course_code IN ('GST101', 'GST102', 'GST201', 'GST202')`
  );
  console.log("\nRemaining duplicate GST courses: " + remainCheck.rows.length + " (should be 0)");
