...
export function VideoPlayer({ url, src, title }: VideoPlayerProps) {
      ...
      if (videoUrl.startsWith("s3://")) {
        try {
          ...
          const response = await fetch(
            `/api/files/signed-url?url=${encodeURIComponent(videoUrl)}`,
          );
  ...
  return (
    ...
    >
      ...
      {/* Controls */}
      <div
        className={`absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black to-transparent p-4 transition-opacity duration-300 ${showControls && duration > 0 ? "opacity-100" : "opacity-0"}`}
      >
        {/* Progress Bar */}
        <div
          ref={progressRef}
