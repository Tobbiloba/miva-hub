...
export async function PUT(
  ...
  try {
    ...
    const [existing] = await pgDb
      ...
      .where(and(eq(AnnouncementSchema.id, id), eq(AnnouncementSchema.createdById, session.user.id)))
      .limit(1);
...
export async function DELETE(
  ...
  try {
    ...
    return NextResponse.json(
      { error: "Failed to delete announcement", message: error instanceof Error ? error.message : "Unknown error" },
      { status: 500 }
    );
  }
}
