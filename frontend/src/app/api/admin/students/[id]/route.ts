...
export async function PUT(
  ...
  try {
    ...
    // Tenant check: changed emails must stay on the admin's university domains
    if (
      validatedData.email &&
      validatedData.email !== existingStudent[0].email
    ) {
      ...
      if (
        !adminUniversity ||
        !emailMatchesUniversity(validatedData.email, adminUniversity)
      ) {
        return NextResponse.json(
          {
            success: false,
            error: adminUniversity
              ? `Email must use one of your university's domains: ${adminUniversity.emailDomains.join(", ")}`
              : "No university associated with your admin account",
          },
          { status: adminUniversity ? 400 : 403 },
        );
      }
    }
    ...
    if (
      ...
      validatedData.studentId !== (existingStudent[0] as any).studentId
    ) {
