...
export async function GET(
  ...
  try {
    ...
    // Fetch department by ID — tenant-checked (prevents cross-university IDOR)
    const department = await pgAcademicRepository.getDepartmentById(id);

    if (
      !department ||
      !(await isSameTenant(adminAccess.user.id, department.universityId))
    ) {
...
export async function PUT(
  ...
  try {
    ...
    // Check if department exists — tenant-checked (prevents cross-university IDOR)
    ...
    if (
      ...
      !(await isSameTenant(
        adminAccess.user.id,
        existingDepartment.universityId,
      ))
    ) {
      return NextResponse.json(
        {
    ...
    if (validatedData.code && validatedData.code !== existingDepartment.code) {
      const duplicateCode = await pgAcademicRepository.getDepartmentByCode(
        validatedData.code,
      );
