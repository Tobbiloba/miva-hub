  ...
  const fetchProcessingJobs = async (status = statusFilter) => {
    try {
      ...
      const response = await fetch(
        `/api/content/processing-jobs?status=${status}&limit=100`,
      );

      if (!response.ok) {
        throw new Error(
          `Failed to fetch processing jobs: ${response.statusText}`,
        );
      }
      ...
      setError(
        err instanceof Error ? err.message : "Failed to fetch processing jobs",
      );
          ...
          <div className="flex items-center gap-4">
            <div className="flex-1">
              <div className="relative">
                ...
                <Input
                  placeholder="Search by job ID, material title, or course..."
                  className="pl-10"
                />
              </div>
            </div>
