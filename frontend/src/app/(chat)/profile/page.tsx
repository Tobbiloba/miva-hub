...
import { getStudentId, getUserRole } from "@/lib/auth/user-utils";
import { pgDb } from "@/lib/db/pg/db.pg";
import { pgAcademicRepository } from "@/lib/db/pg/repositories/academic-repository.pg";
import { UserSchema } from "@/lib/db/pg/schema.pg";
import { eq } from "drizzle-orm";
import { Activity, BookOpen, CreditCard, Settings, User } from "lucide-react";
...
async function getCompleteProfileData(
  userId: string,
...
export default async function ProfilePage() {
  ...
  if (!profileData) {
    console.warn(
      "Profile data fetch failed, using fallback data for user:",
      user.id,
    );
    ...
    return (
      <ProfilePageContent profileData={fallbackData} userRole={userRole} />
    );
  }
      ...
      <div className="container mx-auto px-4 py-6 md:px-6 md:py-8">
        <Tabs defaultValue="overview" className="space-y-6">
          <TabsList className="grid grid-cols-5 w-full max-w-2xl bg-muted/30 border border-border/40">
            <TabsTrigger
              value="overview"
              className="data-[state=active]:bg-background text-xs sm:text-sm flex-col sm:flex-row"
            >
                ...
                <div className="grid gap-4 sm:gap-6 md:grid-cols-2">
                  <div className="space-y-4">
                    <div>
                      ...
                      <div className="mt-1 p-3 bg-muted/30 border border-border/40 rounded-md text-sm">
                        {(profileData.role?.charAt(0)?.toUpperCase() ?? "") +
                          (profileData.role?.slice(1) ?? "") || "Not specified"}
                      </div>
                    </div>
                    {profileData.studentId && (
                      <div>
                      ...
                      <div className="mt-1 p-3 bg-muted/30 border border-border/40 rounded-md text-sm">
                        ...
                        >
                          {(profileData.enrollmentStatus
                            ?.charAt(0)
                            ?.toUpperCase() ?? "") +
                            (profileData.enrollmentStatus?.slice(1) ?? "")}
                        </Badge>
                      </div>
                    </div>
                    <div>
