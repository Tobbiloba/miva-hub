      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Welcome back, {formatPosition(facultyRecord.position)}{" "}
            {facultyInfo.name}
          </p>
        </div>
      ...
      <div className="grid gap-6 md:grid-cols-2">
        ...
        <Card>
          ...
          <CardContent>
            <div className="space-y-3">
              {facultyCourses
                .slice(0, 4)
                .map(({ course, department, courseInstructor }) => (
                  <div
                    key={course.id}
        ...
        <Card>
          ...
          <CardContent>
            <div className="space-y-3">
              {gradingQueue.map(
                ({ submission, assignment, course, student }) => (
                  <div
                    key={submission.id}
                    className="flex items-center justify-between p-3 border rounded-lg"
                  >
              ...
              {dashboardStats.pendingGrades > 0 && (
                <div className="flex items-center justify-between p-3 bg-orange-50 dark:bg-orange-950 rounded-lg">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                      {dashboardStats.pendingGrades} submission
                      {dashboardStats.pendingGrades !== 1 ? "s" : ""} waiting
                      for grades
                    </p>
                  </div>
                  <Button variant="outline" size="sm" asChild>
                    <Link href="/faculty/grades">Grade Now</Link>
                  </Button>
                </div>
              )}
              {dashboardStats.recentSubmissions > 0 && (
                <div className="flex items-center justify-between p-3 bg-blue-50 dark:bg-blue-950 rounded-lg">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                      {dashboardStats.recentSubmissions} new submission
                      {dashboardStats.recentSubmissions !== 1 ? "s" : ""} in the
                      last 7 days
                    </p>
                  </div>
                  <Button variant="outline" size="sm" asChild>
                    <Link href="/faculty/assignments">Review</Link>
                  </Button>
                </div>
              )}
            </div>
