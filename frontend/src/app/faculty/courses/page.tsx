...
export default async function FacultyCoursesPage() {
  ...
  const coursesWithStats = await Promise.all(
    currentSemesterCourses.map(
      async ({ course, department, courseInstructor }) => {
              ...
              <div>
                <p className="text-2xl font-bold">
                  {currentCourses.reduce(
                    (sum, course) => sum + course.stats.enrolledStudents,
                    0,
                  )}
                </p>
          ...
          <CardContent className="p-4">
            <div className="flex items-center gap-2">
              ...
              <div>
                <p className="text-2xl font-bold">
                  {currentCourses.reduce(
                    (sum, course) => sum + course.stats.totalAssignments,
                    0,
                  )}
                </p>
              ...
              <div>
                <p className="text-2xl font-bold">
                  {currentCourses.length > 0
                    ? (
                        currentCourses.reduce(
                          (sum, course) => sum + course.stats.averageGrade,
                          0,
...
function CurrentCoursesList({ courses }: { courses: any[] }) {
  ...
  return (
    <div className="grid gap-6 md:grid-cols-2">
      {courses.map(
        ({ course, department, courseInstructor, stats, schedule }) => (
          <Card key={course.id} className="hover:shadow-md transition-shadow">
            ...
            <CardContent className="space-y-4">
              ...
              <div className="flex gap-2 border-t pt-4">
                ...
                <Button size="sm" variant="outline" asChild>
                  <Link
                    href={`/faculty/assignments/create?courseId=${course.id}`}
                  >
        ...
        .map(([semester, semesterCourses]) => (
          <div key={semester}>
            ...
            <div className="grid gap-4 md:grid-cols-3">
              {semesterCourses.map(
                ({ course, department, courseInstructor }) => (
                  <Card
                  ...
                  >
                    <CardContent className="p-4">
                      <div className="flex items-start justify-between mb-2">
                        ...
                        <Button variant="ghost" size="sm" asChild>
                          <Link
                            href={`/faculty/courses/${course.id}?semester=${semester}`}
                          >
                            <ArrowRight className="h-4 w-4" />
                          </Link>
                        </Button>
