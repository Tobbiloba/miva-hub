...
const academicScheduleSchema = z.object({
  ...
  date: z
    .string()
    .optional()
    .describe("Specific date (YYYY-MM-DD) or 'today', 'tomorrow', 'this-week'"),
  courseCode: z.string().optional().describe("Filter by specific course"),
  eventType: z
    .enum(["class", "assignment", "announcement", "all"])
    .optional()
    .default("all")
    .describe("Type of events to show"),
  daysAhead: z
    .number()
    .optional()
    .default(7)
 ...
 */
async function getAssignmentEvents(
  courses: any[],
  startDate: Date,
  endDate: Date,
): Promise<ScheduleEvent[]> {
 ...
 */
async function getAnnouncementEvents(
  courses: any[],
  startDate: Date,
  endDate: Date,
): Promise<ScheduleEvent[]> {
...
function generateScheduleSummary(
  ...
  const parts: string[] = [];
  if (eventCounts.class)
    parts.push(
      `${eventCounts.class} class${eventCounts.class > 1 ? "es" : ""}`,
    );
  if (eventCounts.assignment)
    parts.push(
      `${eventCounts.assignment} assignment${eventCounts.assignment > 1 ? "s" : ""} due`,
    );
  if (eventCounts.announcement)
    parts.push(
      `${eventCounts.announcement} announcement${eventCounts.announcement > 1 ? "s" : ""}`,
    );
