...
export function isVideoUrl(url: string): boolean {
  ...
  console.log(
    "        - Result:",
    isVideo ? "✅ HAS VIDEO EXTENSION" : "❌ NO VIDEO EXTENSION",
  );
...
export function isVideoMaterial(material: any): boolean {
  ...
  const hasVideoType =
    material.material_type && /video/i.test(material.material_type);
...
export function getVideoDisplayName(material: any): string {
  if (
    material.title &&
    material.title !== "" &&
    material.title !== "Introductory video"
  ) {
...
export function isDocumentMaterial(material: any): boolean {
  ...
  const isDoc = /\.(pdf|doc|docx|ppt|pptx|xls|xlsx|txt)$/i.test(
    material.file_url,
  );
  console.log(
    "      - isDocument:",
    isDoc ? "✅ IS DOCUMENT" : "❌ NOT DOCUMENT",
  );
  return isDoc;
}
...
export function extractRichMaterials(toolOutput: any): any[] {
  ...
  if (toolOutput?.structuredContent?.result) {
    ...
    try {
      ...
      if (parsed?.materials?.length) {
        console.log(
          "    ✅ Found materials in structuredContent.result:",
        ...
        const richMaterials = parsed.materials.filter((material, index) => {
          ...
          console.log(
            "    - Result for #" + index + ":",
            isRich ? "✅ IS RICH CONTENT" : "❌ NOT RICH CONTENT",
          );
          return isRich;
        });
        console.log(
          "    📊 Path 1 result:",
          richMaterials.length,
  ...
  if (toolOutput?.content?.[0]?.text?.materials?.length) {
    ...
    console.log(
      "    ✅ Found materials in content[0].text.materials:",
      materials.length,
      "items",
    );
  ...
  if (toolOutput?.materials?.length) {
    ...
    console.log(
      "    ✅ Found materials in direct .materials property:",
      toolOutput.materials.length,
      "items",
    );
  ...
  console.log(
    "    2. content[0].text.materials:",
    !!toolOutput?.content?.[0]?.text?.materials,
  );
